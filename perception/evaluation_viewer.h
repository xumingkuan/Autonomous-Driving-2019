...
class EvaluationViewer : public utils::display::PainterWidgetBase {
 ...
 private:
  struct PolygonInfo {
    ...
    mutable utils::Optional<math::Vec2d> center;
    math::Vec2d GetCenter() const {
      if (!center) {
        center = std::accumulate(polygon.begin(), polygon.end(), math::Vec2d(0, 0),
            [](const math::Vec2d& sum, const math::Vec2d& point) { return sum + point; }) /
